...
function enableNotifications() {
  ...
  if (Notification.permission === "denied") {
    alert(
      "Browser denied permission to send desktop notifications.\n" +
...
function sendNotification(title, options = {}) {
  ...
  if (options.tag !== null && senttags.indexOf(options.tag) >= 0) {
    return;
  }
  ...
  for (var name in names) {
    ...
    for (var i = 0; i < cells.length; i++) {
      ...
      cells[i].style.display =
        cells[i].style.display === "none" ? style : "none";
    }
  }
}
...
function updateClock() {
  ...
  var fmt = "";
  if (
    timeleftelt.innerHTML == "start delayed" ||
    timeleft.innerHTML == "no contest"
  ) {
    // FIXME
    var left = 0;
...
function setCookie(name, value) {
  ...
  document.cookie =
    name + "=" + escape(value) + "; expires=" + expire.toUTCString();
}
...
function postVerifyCommentToICAT(url, user, teamid, probid, submissionid) {
  ...
  addField(
    "text",
    "Team #t" + teamid + " on problem #p" + probid + ": " + msg.value,
  );
...
function processAjaxResponse(jqXHR, data) {
  if (jqXHR.getResponseHeader("X-Login-Page")) {
    ...
    var newCurrentContest = parseInt(
      jqXHR.getResponseHeader("X-Current-Contest"),
    );
    var dataCurrentContest = $("[data-current-contest]").data(
      "current-contest",
    );
    ...
    if (
      dataCurrentContest !== undefined &&
      newCurrentContest !== currentContest
    ) {
      window.location.reload();
      return;
    }
  ...
  if (num == 0) {
    ...
    for (var i = 0; i < num; i++) {
      sendNotification("Judgehost down.", {
        tag: "host_" + data[i].hostname + "@" + Math.floor(data[i].polltime),
        link:
          domjudge_base_url +
          "/jury/judgehosts/" +
          encodeURIComponent(data[i].hostname),
