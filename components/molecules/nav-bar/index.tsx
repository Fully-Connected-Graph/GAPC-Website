...
export const NavBar: FC = async () => {
  ...
  return (
    <nav className="fixed top-0 h-20 w-screen mb-4 flex items-center justify-center z-10 bg-black bg-opacity-[50] bg-blend-soft-light shadow-sm shadow-white/30 ">
      <div className="w-full max-w-5xl flex gap-4 items-center text-gray-100 max-lg:px-4 max-lg:max-w-full">
        ...
        {links.map((link, index) => (
          ...
          >
            {link.name}
            <span className="block mt-1 max-w-0 group-hover:max-w-full duration-200 h-0.5 bg-gradient-to-r from-white to-gray-400"></span>
          </Link>
        ))}
